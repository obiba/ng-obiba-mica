...
this.newCriteriaItemBuilder=function(){return new CriteriaItemBuilder(d,e)},this.initialize=function(f){this.leafItemMap={},this.target=f,this.rootRql=a,this.taxonomies=c,this.LocalizedValues=d,this.lang=e,this.rootItem=this.newCriteriaItemBuilder().parent(b).type(this.target).rqlQuery(this.rootRql).target(this.target).build()},/**
   ...
   * @param node
   */
this.buildLeafItem=function(a,b,c,d,e){var f=this,g=new CriteriaItemBuilder(f.LocalizedValues,f.lang).type(d.name).target(f.target).taxonomy(a).vocabulary(b).rqlQuery(d).parent(e);return g.selectedTerms(c).build(),g.build()}}/* global CRITERIA_ITEM_EVENT */
 ...
 */
CriteriaBuilder.prototype.fieldToVocabulary=function(a){var b={taxonomy:null,vocabulary:null},c=a;a.indexOf(".")<0&&(c="Mica_"+this.target+"."+a);var d=c.split(".",2),e=d[0],f=d[1],g=this.taxonomies.filter(function(a){return e===a.name});if(0===g.length)throw new Error("Could not find taxonomy:",e);b.taxonomy=g[0];var h=b.taxonomy.vocabularies.filter(function(a){return f===a.name});if(0===h.length)throw new Error("Could not find vocabulary:",f);return b.vocabulary=h[0],b},/**
 * This method is where a criteria gets created
 */
CriteriaBuilder.prototype.visitLeaf=function(a,b){var c=a.args[0],d=a.args[1],e=this.fieldToVocabulary(c),f=this.buildLeafItem(e.taxonomy,e.vocabulary,d instanceof Array?d:[d],a,b);this.leafItemMap[f.id]=f,b.children.push(f)},/**
 * Returns all the criterias found
 * @returns {Array}
 */
CriteriaBuilder.prototype.getRootItem=function(){return this.rootItem},/**
 * Returns the leaf criteria item map needed for finding duplicates
 * @returns {Array}
 */
CriteriaBuilder.prototype.getLeafItemMap=function(){return this.leafItemMap},/**
 * Node condition visitor
 * @param node
 * @param parentItem
 */
CriteriaBuilder.prototype.visitCondition=function(a,b){var c=this.newCriteriaItemBuilder().parent(b).rqlQuery(a).type(a.name).build();b.children.push(c),this.visit(a.args[0],c),this.visit(a.args[1],c)},/**
 * Node not visitor
 * @param node
 * @param parentItem
 */
CriteriaBuilder.prototype.visitNot=function(a,b){var c=this.newCriteriaItemBuilder().parent(b).rqlQuery(a).type(a.name).build();b.children.push(c),this.visit(a.args[0],c)},/**
 * General purpose node visitor
 * @param node
 * @param parentItem
 */
CriteriaBuilder.prototype.visit=function(a,b){
// TODO needs to add more types
switch(a.name){case RQL_NODE.NOT:this.visitNot(a,b);break;case RQL_NODE.AND:case RQL_NODE.NAND:case RQL_NODE.OR:case RQL_NODE.NOR:this.visitCondition(a,b);break;case RQL_NODE.IN:case RQL_NODE.OUT:case RQL_NODE.EQ:case RQL_NODE.LE:case RQL_NODE.LT:case RQL_NODE.GE:case RQL_NODE.GT:case RQL_NODE.BETWEEN:case RQL_NODE.EXISTS:case RQL_NODE.MISSING:this.visitLeaf(a,b);break;case RQL_NODE.MATCH:}},/**
 * Builds a criteria list for this target
 */
CriteriaBuilder.prototype.build=function(){var a=this;this.rootRql.args.forEach(function(b){a.visit(b,a.rootItem)})},angular.module("obiba.mica.search").service("RqlQueryUtils",[function(){/**
     * Finds the parent node to which new queries can be added
     *
     * @param targetNode
     * @returns {*}
     */
function a(a){var b=a.args.filter(function(a){switch(a.name){case RQL_NODE.AND:case RQL_NODE.NAND:case RQL_NODE.OR:case RQL_NODE.NOR:case RQL_NODE.NOT:case RQL_NODE.IN:case RQL_NODE.OUT:case RQL_NODE.EQ:case RQL_NODE.GT:case RQL_NODE.GE:case RQL_NODE.LT:case RQL_NODE.LE:case RQL_NODE.BETWEEN:case RQL_NODE.MATCH:case RQL_NODE.EXISTS:case RQL_NODE.MISSING:return!0}return!1}).pop();return b?a.args.findIndex(function(a){return a.name===b.name}):-1}function b(a,b,c){var d=c;return a.attributes&&a.attributes.some(function(a){return a.key===b?(d=a.value,!0):!1}),d}var c=this;this.vocabularyTermNames=function(a){return a&&a.terms?a.terms.map(function(a){return a.name}):[]},this.hasTargetQuery=function(a){return a.args.filter(function(a){switch(a.name){case RQL_NODE.VARIABLE:case RQL_NODE.DATASET:case RQL_NODE.STUDY:case RQL_NODE.NETWORK:return!0;default:return!1}}).length>0},this.variableQuery=function(){return new RqlQuery(QUERY_TARGETS.VARIABLE)},this.eqQuery=function(a,b){var c=new RqlQuery(RQL_NODE.EQ);return c.args.push(b),c},this.aggregate=function(a){var b=new RqlQuery(RQL_NODE.AGGREGATE);return a.forEach(function(a){b.args.push(a)}),b},this.limit=function(a,b){var c=new RqlQuery(RQL_NODE.LIMIT);return c.args.push(a),c.args.push(b),c},this.inQuery=function(a,b){var c=b&&b.length>0,d=c?RQL_NODE.IN:RQL_NODE.EXISTS,e=new RqlQuery(d);return e.args.push(a),c&&e.args.push(b),e},this.rangeQuery=function(a,b,d){var e=new RqlQuery(RQL_NODE.BETWEEN);return e.args.push(a),c.updateRangeQuery(e,b,d),e},this.updateInQuery=function(a,b,c){var d=b&&b.length>0;return a.name=d?RQL_NODE.IN:c?RQL_NODE.MISSING:RQL_NODE.EXISTS,d?a.args[1]=b:a.args.splice(1,1),a},this.mergeInQueryArgValues=function(a,b){var c=b&&b.length>0;if(a.name=c?RQL_NODE.IN:RQL_NODE.EXISTS,c){var d=a.args[1];if(d){d instanceof Array||(d=[d]);var e=b.filter(function(a){return-1===d.indexOf(a)});a.args[1]=d.concat(e)}else a.args[1]=b}else a.args.splice(1,1);return a},this.updateRangeQuery=function(a,b,c,d){d?(a.name=RQL_NODE.MISSING,a.args.splice(1,1)):angular.isDefined(b)&&null!==b&&angular.isDefined(c)&&null!==c?(a.name=RQL_NODE.BETWEEN,a.args[1]=[b,c]):angular.isDefined(b)&&null!==b?(a.name=RQL_NODE.GE,a.args[1]=b):angular.isDefined(c)&&null!==c?(a.name=RQL_NODE.LE,a.args[1]=c):(a.name=RQL_NODE.EXISTS,a.args.splice(1,1))},/**
     * Creates a RqlQuery from an item
     *
     * @param item
     * @returns {RqlQuery}
     */
this.buildRqlQuery=function(a){return c.isNumericVocabulary(a.vocabulary)?this.rangeQuery(this.criteriaId(a.taxonomy,a.vocabulary),null,null):this.inQuery(this.criteriaId(a.taxonomy,a.vocabulary),a.term?a.term.name:void 0)},/**
     ...
     * @returns {*}
     */
this.addQuery=function(b,c){if(0===b.args.length)b.args.push(c);else{var d=a(b);if(-1===d)b.args.push(c);else{var e=b.args.splice(d,1).pop(),f=new RqlQuery(RQL_NODE.OR);f.args.push(e,c),b.args.push(f)}}return b},this.updateQueryArgValues=function(a,b){switch(a.name){case RQL_NODE.IN:case RQL_NODE.EXISTS:this.mergeInQueryArgValues(a,b)}},this.updateQuery=function(a,b,c){switch(a.name){case RQL_NODE.IN:case RQL_NODE.EXISTS:case RQL_NODE.MISSING:this.updateInQuery(a,b,c)}},this.addLocaleQuery=function(a,b){var c=a.args.filter(function(a){return a.name===RQL_NODE.LOCALE}).pop();if(!c){var d=new RqlQuery("locale");d.args.push(b),a.args.push(d)}},this.addLimit=function(a,b){var c=a.args.filter(function(a){return a.name===RQL_NODE.LIMIT}).pop();c?c.args=b.args:a.args.push(b)},/**
     ...
     * @returns {*}
     */
this.criteriaId=function(a,b){return a.name+"."+b.name},this.vocabularyType=function(a){return b(a,"type",VOCABULARY_TYPES.STRING)},this.vocabularyField=function(a){return b(a,"field",a.name)},this.vocabularyAlias=function(a){return b(a,"alias",a.name)},this.isNumericVocabulary=function(a){return!a.terms&&(c.vocabularyType(a)===VOCABULARY_TYPES.INTEGER||c.vocabularyType(a)===VOCABULARY_TYPES.DECIMAL)}}]).service("RqlQueryService",["$q","TaxonomiesResource","LocalizedValues","RqlQueryUtils",function(a,b,c,d){function e(a,b){return b.args.filter(function(b){return b.name===a}).pop()}function f(a){switch(a.type){case RQL_NODE.IN:case RQL_NODE.OUT:case RQL_NODE.EQ:case RQL_NODE.GT:case RQL_NODE.GE:case RQL_NODE.LT:case RQL_NODE.LE:case RQL_NODE.BETWEEN:case RQL_NODE.MATCH:case RQL_NODE.EXISTS:case RQL_NODE.MISSING:return!0}return!1}function g(a){var b=a.parent,c=a.rqlQuery,d=c.args,e=a.parent.rqlQuery,f=e.args.indexOf(c);if(-1===f)throw new Error("Criteria node not found: "+a);e.args.splice(f,1),d&&(d instanceof Array?e.args.splice.apply(e.args,[f,0].concat(d)):e.args.splice(f,0,d)),null!==b.parent&&0===e.args.length&&g(b)}function h(a){var b=a.parent,c=a.rqlQuery,d=c.args,e=a.parent.rqlQuery,f=e.args.indexOf(c);if(-1===f)throw new Error("Criteria node not found: "+a);e.args.splice(f,1),d&&(d instanceof Array?e.args.splice.apply(e.args,[f,0].concat(d)):e.args.splice(f,0,d)),0===e.args.length&&g(b)}function i(a){var b=a.parent;if(!b)throw new Error("Cannot remove criteria when parent is NULL");var c=a.rqlQuery,d=a.parent.rqlQuery,e=d.args.indexOf(c);if(-1===e)throw new Error("Criteria node not found: "+a);d.args.splice(e,1),-1!==[RQL_NODE.OR,RQL_NODE.AND,RQL_NODE.NAND,RQL_NODE.NOR].indexOf(b.type)?h(b):0===d.args.length&&g(b)}var j={variable:null,dataset:null,study:null,network:null};/**
       * Removes the item from criteria item tree. This should be from a leaf.
       * @param item
       */
this.removeCriteriaItem=function(a){f(a)&&i(a)},/**
       ...
       */
this.createCriteriaItem=function(a,b,d,e,f){return new CriteriaItemBuilder(c,f).target(a).taxonomy(b).vocabulary(d).term(e).build()},/**
       * Adds new item to the item tree
       *
       * @param rootItem
       * @param item
       */
this.addCriteriaItem=function(a,b){var c=a.args.filter(function(a){return b.target===a.name}).pop();c||(c=new RqlQuery(RQL_NODE[b.target.toUpperCase()]),a.args.push(c));var e=d.buildRqlQuery(b);return d.addQuery(c,e)},/**
       * Update an exising item to the item tree
       *
       * @param rootItem
       * @param item
       */
this.updateCriteriaItem=function(a,b){d.updateQueryArgValues(a.rqlQuery,b.term?[b.term.name]:d.vocabularyTermNames(b.vocabulary))},/**
       * Builders registry
       *
       * @type {{variable: builders.variable, study: builders.study}}
       */
this.builders=function(d,e,f,g){function h(a,b){var e=new CriteriaBuilder(a,b,j[d],c,g);e.initialize(d),e.build(),i.resolve({root:e.getRootItem(),map:e.getLeafItemMap()})}var i=a.defer();return j[d]?h(e,f):b.get({target:d}).$promise.then(function(a){j[d]=a,h(e,f)}),i.promise},/**
       ...
       * @returns {*}
       */
this.createCriteria=function(b,c){var e=a.defer(),f=(new CriteriaItemBuilder).type(RQL_NODE.AND).rqlQuery(b).build(),g={};if(!d.hasTargetQuery(b))return e.resolve({root:f,map:g}),e.promise;var h=[],i=this,j=0;return b.args.forEach(function(a){QUERY_TARGETS[a.name.toUpperCase()]&&h.push(a)}),h.forEach(function(a){i.builders(a.name,a,f,c).then(function(a){f.children.push(a.root),g=angular.extend(g,a.map),j++,j===h.length&&e.resolve({root:f,map:g})})}),e.promise},/**
       ...
       */
this.prepareCriteriaTermsQuery=function(a,b,c){var e=(new RqlParser).parse(a),f=e.args.filter(function(a){return a.name===b.target}).pop();return f&&(f.args.push(d.aggregate([d.criteriaId(b.taxonomy,b.vocabulary)])),f.args.push(d.limit(0,0))),e.args.push(new RqlQuery(RQL_NODE.FACET)),c&&d.addLocaleQuery(e,c),e.serializeArgs(e.args)},this.prepareSearchQuery=function(a,b,c,f){var g=angular.copy(b),h=typeToTarget(a);d.addLocaleQuery(g,f);var i=e(h,g);i||(i=new RqlQuery(h),g.args.push(i));var j=c[h]||{from:0,size:10};return d.addLimit(i,d.limit(j.from,j.size)),(new RqlQuery).serializeArgs(g.args)},/**
       ...
       * @returns the new query
       */
this.prepareCoverageQuery=function(a,b){var c=(new RqlParser).parse(a),d=new RqlQuery("aggregate"),e=new RqlQuery("bucket");b.forEach(function(a){e.args.push(a)}),d.args.push(e);var f;return c.args.forEach(function(a){f||"variable"!==a.name||(f=a)}),f||(f=new RqlQuery("variable"),c.args.push(f)),f.args.push(d),c.serializeArgs(c.args)},this.prepareGraphicsQuery=function(a,b){var c=(new RqlParser).parse(a),d=new RqlQuery("aggregate");b.forEach(function(a){d.args.push(a)});
// limit
var e=new RqlQuery("limit");e.args.push(0),e.args.push(0);
// study
var f;
// facet
return c.args.forEach(function(a){"study"===a.name&&(f=a)}),f||(f=new RqlQuery("study"),c.args.push(f)),f.args.push(d),f.args.push(e),c.args.push(new RqlQuery("facet")),c.serializeArgs(c.args)},this.getTargetAggregations=function(a,b,e){/**
