...
angular.module("obiba.mica.search",["obiba.alert","ui.bootstrap","pascalprecht.translate","templates-ngObibaMica"]).config(["$provide",function(a){a.provider("ngObibaMicaSearchTemplateUrl",(new NgObibaMicaTemplateUrlFactory).create({list:{header:null,footer:null},view:{header:null,footer:null},form:{header:null,footer:null}}))}]).run(["GraphicChartsConfigurations",function(a){a.setClientConfig()}]),angular.module("obiba.mica.search").filter("regex",function(){return function(a,b,c){var d=[];try{var e=new RegExp(b);d=a.filter(function(a){return c.some(function(b){return e.test(a[b])})})}catch(f){}return d}}).filter("orderBySelection",function(){return function(a,b){if(!a)return[];var c=[],d=[];return a.forEach(function(a){b[a.key]?c.push(a):d.push(a)}),c.concat(d)}});/* exported QUERY_TYPES */
