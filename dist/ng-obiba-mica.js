...
(function() {
    ...
    ngObibaMica.search.component("matchVocabularyFilterDetail", {
        transclude: true,
        bindings: {
            vocabulary: "<",
            onSelectArgs: "&"
...
(function() {
    ...
    ngObibaMica.search.component("numericVocabularyFilterDetail", {
        transclude: true,
        bindings: {
            vocabulary: "<",
            onSelectArgs: "&"
...
var TermsVocabularyFilterDetailController = function() {
...
var TermsVocabularyFilterDetailComponent = function() {
...
ngObibaMica.search.component("termsVocabularyFilterDetail", new TermsVocabularyFilterDetailComponent());

"use strict";

ngObibaMica.search.controller("MatchVocabularyFacetController", [ "$scope", "RqlQueryService", function($scope, RqlQueryService) {
    ...
    function updateCriteria() {
        $scope.$parent.selectTerm($scope.$parent.target, $scope.$parent.taxonomy, $scope.vocabulary, {
            text: $scope.text || "*"
        });
    }
...
ngObibaMica.search.controller("NumericVocabularyFacetController", [ "$scope", "JoinQuerySearchResource", "RqlQueryService", "RqlQueryUtils", function($scope, JoinQuerySearchResource, RqlQueryService, RqlQueryUtils) {
    function updateLimits(criteria, vocabulary) {
        ...
        JoinQuerySearchResource[targetToType($scope.target)]({
            query: joinQuery
        }).$promise.then(function(joinQueryResponse) {
    ...
    $scope.$on("ngObibaMicaLoadVocabulary", function(ev, taxonomy, vocabulary) {
        if ($scope.vocabulary.isNumeric && vocabulary.name === $scope.vocabulary.name && !vocabulary.isOpen) {
...
ngObibaMica.search.controller("TaxonomiesFacetsController", [ "$scope", "$timeout", "TaxonomyResource", "TaxonomiesResource", "LocalizedValues", "ngObibaMicaSearch", "RqlQueryUtils", "VocabularyService", function($scope, $timeout, TaxonomyResource, TaxonomiesResource, LocalizedValues, ngObibaMicaSearch, RqlQueryUtils, VocabularyService) {
    $scope.options = ngObibaMicaSearch.getOptions();
    $scope.taxonomies = {};
    $scope.targets = [];
    $scope.RqlQueryUtils = RqlQueryUtils;
    ...
    function init(target) {
        ...
        TaxonomiesResource.get({
            ...
            $scope.taxonomies[target] = $scope.facetedTaxonomies[target].map(function(f) {
                ...
                t.isOpen = false;
                t.vocabularies = "Maelstrom Research" === t.author ? t.vocabularies : VocabularyService.visibleFacetVocabularies(t.vocabularies);
...
ngObibaMica.search.controller("TermsVocabularyFacetController", [ "$scope", "$filter", "JoinQuerySearchResource", "RqlQueryService", "RqlQueryUtils", function($scope, $filter, JoinQuerySearchResource, RqlQueryService, RqlQueryUtils) {
    ...
    function updateCounts(criteria, vocabulary) {
        ...
        if (RqlQueryUtils.hasTargetQuery(criteria.rqlQuery, criterion.target)) {
            ...
            if (!isCriterionPresent) {
                var operator = criterion.target === QUERY_TARGETS.VARIABLE && criterion.taxonomy.name !== "Mica_variable" ? RQL_NODE.OR : RQL_NODE.AND;
    ...
    $scope.$on("ngObibaMicaLoadVocabulary", function(ev, taxonomy, vocabulary) {
        if (vocabulary.name === $scope.vocabulary.name && !$scope.vocabulary.isNumeric && !$scope.vocabulary.isMatch && !vocabulary.isOpen) {
...
(function() {
    ...
    ngObibaMica.search.component("metaTaxonomyFilterList", {
        transclude: true,
        bindings: {
            metaTaxonomy: "<",
            showTaxonomyPanel: "<",
...
(function() {
    function Controller(MetaTaxonomyService, TaxonomyService) {
        ...
        function onSelectTaxonomy(target, selectedTaxonomy) {
            if (ctrl.selectedTaxonomy !== selectedTaxonomy) {
                ...
                setTimeout(function() {
                    TaxonomyService.getTaxonomies(target, selectedTaxonomy.info.names || selectedTaxonomy.info.name).then(function(taxonomy) {
                ...
                ctrl.onToggle({
                    target: target,
                    taxonomy: ctrl.selectedTaxonomy
                });
            }
        }
        function onChanges(changed) {
    ...
    ngObibaMica.search.component("metaTaxonomyFilterPanel", {
        bindings: {
            tabs: "<",
            showTaxonomyPanel: "<",
            onToggle: "&",
...
(function() {
    ...
    ngObibaMica.search.controller("ClassificationPanelController", [ "$rootScope", "$scope", "$translate", "$location", "TaxonomyResource", "TaxonomiesResource", "ngObibaMicaSearch", "RqlQueryUtils", "$cacheFactory", "VocabularyService", ClassificationPanelController ]).directive("classificationsPanel", [ function() {
        return {
            restrict: "EA",
            replace: true,
            scope: {
...
(function() {
    ...
    ngObibaMica.search.controller("TaxonomiesPanelController", [ "$rootScope", "$scope", "$translate", "$location", "TaxonomyResource", "TaxonomiesResource", "ngObibaMicaSearch", "RqlQueryUtils", "$cacheFactory", "AlertService", "ServerErrorUtils", "VocabularyService", TaxonomiesPanelController ]).controller("NumericVocabularyPanelController", [ "$scope", function($scope) {
...
var CellStatValueController = function() {
...
var CellStatValueComponent = function() {
...
ngObibaMica.search.component("cellStatValue", new CellStatValueComponent());

"use strict";

ngObibaMica.search.controller("CoverageResultTableController", [ "$scope", "$location", "$q", "$translate", "$filter", "LocalizedValues", "PageUrlService", "RqlQueryUtils", "RqlQueryService", "CoverageGroupByService", "StudyFilterShortcutService", "TaxonomyService", "AlertService", function($scope, $location, $q, $translate, $filter, LocalizedValues, PageUrlService, RqlQueryUtils, RqlQueryService, CoverageGroupByService, StudyFilterShortcutService, TaxonomyService, AlertService) {
    ...
    function setInitialFilter() {
        ...
        if (bucket === BUCKET_TYPES.STUDY || bucket === BUCKET_TYPES.DCE) {
            $scope.bucketSelection._dceBucketSelected = bucket === BUCKET_TYPES.DCE;
        }
    }
    ...
    function updateFilterCriteriaInternal(selected) {
        ...
        vocabularyQuery.args.push(selected.map(function(selection) {
            return selection.value;
        }));
        if (!foundVocabularyQuery) {
            if (targetQuery.args.length > 0) {
                var andQuery = new RqlQuery(RQL_NODE.AND);
                targetQuery.args.forEach(function(arg) {
                    andQuery.args.push(arg);
                });
        ...
        TaxonomyService.findVocabularyInTaxonomy(targetMap[$scope.bucket], "Mica_" + targetMap[$scope.bucket], vocabulary).then(function(foundVocabulary) {
            AlertService.growl({
                id: "SearchControllerGrowl",
                type: "info",
                msgKey: growlMsgKey,
                msgArgs: [ foundVocabulary ? LocalizedValues.forLocale(foundVocabulary.title, $translate.use()) : vocabulary, $filter("translate")("taxonomy.target." + targetMap[$scope.bucket]) ],
                delay: 3e3
            });
        });
    }
    function splitIds() {
        ...
        $scope.result.rows.forEach(function(row, i) {
            ...
            if ($scope.bucket.startsWith("dce")) {
                ...
                var isHarmo = row.className.indexOf("Harmonization") > -1 || ids[2] === ".";
    ...
    $scope.updateCriteria = function(id, term, idx, type) {
        var vocabulary = $scope.bucket.startsWith("dce") ? "dceId" : "id";
        var criteria = {
            varItem: RqlQueryService.createCriteriaItem(QUERY_TARGETS.VARIABLE, term.taxonomyName, term.vocabularyName, term.entity.name)
        };
        if (id) {
    ...
    $scope.isFullCoverageImpossibleOrCoverageAlreadyFull = function() {
        ...
        rows.forEach(function(row) {
            if (row.hits) {
                if (row.hits.filter(function(hit) {
                    return hit === 0;
                }).length > 0) {
...
(function() {
    ...
    ngObibaMica.search.directive("datasetsResultTable", [ "$log", "PageUrlService", "ngObibaMicaSearch", "TaxonomyResource", "RqlQueryService", "ngObibaMicaSearchTemplateUrl", DatasetsResultTable ]);
})();

"use strict";

ngObibaMica.search.controller("GraphicsResultController", [ "GraphicChartsConfig", "GraphicChartsUtils", "RqlQueryService", "$filter", "$scope", "D3GeoConfig", "D3ChartConfig", function(GraphicChartsConfig, GraphicChartsUtils, RqlQueryService, $filter, $scope, D3GeoConfig, D3ChartConfig) {
    ...
    var setChartObject = function(vocabulary, dtoObject, header, title, options, isTable) {
        return GraphicChartsUtils.getArrayByAggregation(vocabulary, dtoObject).then(function(entries) {
            var data = entries.map(function(e) {
    ...
    $scope.$watch("result", function(result) {
        ...
        if (result && result.studyResultDto.totalHits) {
            ...
            setChartObject("populations-model-selectionCriteria-countriesIso", result.studyResultDto, [ $filter("translate")(charOptions.geoChartOptions.header[0]), $filter("translate")(charOptions.geoChartOptions.header[1]) ], $filter("translate")(charOptions.geoChartOptions.title) + " (N=" + result.studyResultDto.totalHits + ")", charOptions.geoChartOptions.options).then(function(geoStudies) {
                if (geoStudies) {
                    var d3Config = new D3GeoConfig().withData(geoStudies.entries).withTitle($filter("translate")(charOptions.geoChartOptions.title) + " (N=" + result.studyResultDto.totalHits + ")");
            ...
            setChartObject("model-methods-design", result.studyResultDto, [ $filter("translate")(charOptions.studiesDesigns.header[0]), $filter("translate")(charOptions.studiesDesigns.header[1]) ], $filter("translate")(charOptions.studiesDesigns.title) + " (N=" + result.studyResultDto.totalHits + ")", charOptions.studiesDesigns.options).then(function(methodDesignStudies) {
                if (methodDesignStudies) {
                    var d3Config = new D3ChartConfig(methodDesignStudies.vocabulary).withType("multiBarHorizontalChart").withTitle($filter("translate")(charOptions.studiesDesigns.title) + " (N=" + result.studyResultDto.totalHits + ")").withData(angular.copy(methodDesignStudies.entries), false, $filter("translate")("graphics.nbr-studies"));
            ...
            setChartObject("model-methods-design", result.studyResultDto, [ $filter("translate")(charOptions.studiesDesigns.header[0]), $filter("translate")(charOptions.studiesDesigns.header[1]), $filter("translate")(charOptions.studiesDesigns.header[2]) ], $filter("translate")(charOptions.studiesDesigns.title) + " (N=" + result.studyResultDto.totalHits + ")", charOptions.studiesDesigns.options, true).then(function(methodDesignStudies) {
                if (methodDesignStudies) {
                    var chartObject = {
                        chartObjectTable: {
                            options: methodDesignStudies.options,
            ...
            setChartObject("model-numberOfParticipants-participant-number-range", result.studyResultDto, [ $filter("translate")(charOptions.numberParticipants.header[0]), $filter("translate")(charOptions.numberParticipants.header[1]) ], $filter("translate")(charOptions.numberParticipants.title) + " (N=" + result.studyResultDto.totalHits + ")", charOptions.numberParticipants.options).then(function(numberParticipant) {
                if (numberParticipant) {
                    var chartConfig = new D3ChartConfig(numberParticipant.vocabulary).withType("pieChart").withTitle($filter("translate")(charOptions.numberParticipants.title) + " (N=" + result.studyResultDto.totalHits + ")").withData(angular.copy(numberParticipant.entries), true);
            ...
            setChartObject("populations-dataCollectionEvents-model-bioSamples", result.studyResultDto, [ $filter("translate")(charOptions.biologicalSamples.header[0]), $filter("translate")(charOptions.biologicalSamples.header[1]) ], $filter("translate")(charOptions.biologicalSamples.title) + " (N=" + result.studyResultDto.totalHits + ")", charOptions.biologicalSamples.options).then(function(bioSamplesStudies) {
                if (bioSamplesStudies) {
                    var d3Config = new D3ChartConfig(bioSamplesStudies.vocabulary).withType("multiBarHorizontalChart").withTitle($filter("translate")(charOptions.biologicalSamples.title) + " (N=" + result.studyResultDto.totalHits + ")").withData(angular.copy(bioSamplesStudies.entries), false, $filter("translate")("graphics.nbr-studies"));
                    d3Config.options.chart.showLegend = false;
                    d3Config.options.chart.color = charOptions.numberParticipants.options.colors;
                    var chartObject = {
                        biologicalSamples: {
...
(function() {
    ...
    ngObibaMica.search.directive("networksResultTable", [ "PageUrlService", "ngObibaMicaSearch", "RqlQueryService", "StudyFilterShortcutService", "ngObibaMicaSearchTemplateUrl", NetworksResultTable ]);
})();

"use strict";

var SearchResultPaginationController = function() {
...
var SearchResultPaginationComponent = function() {
...
ngObibaMica.search.controller("SearchResultController", [ "$scope", "ngObibaMicaSearch", "ngObibaMicaUrl", "RqlQueryService", "RqlQueryUtils", "ngObibaMicaSearchTemplateUrl", "AlertService", "SetService", function($scope, ngObibaMicaSearch, ngObibaMicaUrl, RqlQueryService, RqlQueryUtils, ngObibaMicaSearchTemplateUrl, AlertService, SetService) {
    ...
    $scope.options = ngObibaMicaSearch.getOptions();
    $scope.activeTarget = {};
    $scope.activeTarget[QUERY_TYPES.VARIABLES] = {
        active: false,
        name: QUERY_TARGETS.VARIABLE
    };
    $scope.activeTarget[QUERY_TYPES.DATASETS] = {
        active: false,
        name: QUERY_TARGETS.DATASET
    };
    $scope.activeTarget[QUERY_TYPES.STUDIES] = {
        active: false,
        name: QUERY_TARGETS.STUDY
    };
    $scope.activeTarget[QUERY_TYPES.NETWORKS] = {
        active: false,
        name: QUERY_TARGETS.NETWORK
    };
    ...
    $scope.getReportUrl = function() {
        ...
        var queryWithLimit = rewriteQueryWithLimitAndFields(1e5);
    ...
    $scope.addToCart = function() {
        ...
        var beforeCart = SetService.getCartSet("variables");
        var queryWithLimit = rewriteQueryWithLimitAndFields(2e4, [ "id" ]);
    ...
    $scope.getStudySpecificReportUrl = function() {
        ...
        RqlQueryUtils.addLimit(targetQuery, RqlQueryUtils.limit(0, 1e5));
...
(function() {
        ...
        return {
            ...
            link: function(scope) {
                ...
                function getDatasourceTitles() {
                    if (Object.keys(scope.taxonomy).length < 1 || !scope.taxonomy.vocabularies || Object.keys(scope.datasourceTitles).length > 0) {
                        return;
                    }
                    scope.taxonomy.vocabularies.some(function(vocabulary) {
                        if (vocabulary.name === "populations-dataCollectionEvents-dataSources") {
                            vocabulary.terms.forEach(function(term) {
                                scope.datasourceTitles[term.name] = {
                                    title: LocalizedValues.forLocale(term.title, scope.lang)
                                };
                            });
                            return true;
                        }
    ...
    ngObibaMica.search.directive("studiesResultTable", [ "$log", "$q", "$location", "PageUrlService", "ngObibaMicaSearch", "TaxonomyResource", "RqlQueryService", "LocalizedValues", "ngObibaMicaSearchTemplateUrl", "StudyFilterShortcutService", StudiesResultTable ]);
})();
...
(function() {
    function TaxonomyFilterDetailController() {
        ...
        function selectVocabularyArgs(vocabulary, args) {
            ctrl.onSelectTaxonomyTerm({
                taxonomy: ctrl.taxonomy,
                vocabulary: vocabulary,
                args: args
            });
        }
        function removeCriterion(item) {
            ctrl.onRemoveCriterion({
                item: item
            });
        }
    ...
    ngObibaMica.search.component("taxonomyFilterDetail", {
        bindings: {
            taxonomy: "<",
            vocabularies: "<",
...
(function() {
    ...
    ngObibaMica.search.component("taxonomyFilterPanel", {
        transclude: true,
        bindings: {
            showTaxonomyPanel: "<",
            result: "<",
...
(function() {
    function VocabularyFilterDetailController(VocabularyService) {
        ...
        function selectVocabularyArgs(args) {
            if (ctrl.criterionType === "string-terms" && !args.term.selected) {
                ...
                if (selectedTerms.length === 0) {
                    ctrl.onRemoveCriterion({
                        item: ctrl.vocabulary.existingItem
                    });
                } else {
                    ctrl.onSelectVocabularyArgs({
                        vocabulary: ctrl.vocabulary,
                        args: args
                    });
                }
        ...
        function canStillSelectMore(terms) {
            ...
            var selected = terms.filter(function(term) {
                return term.selected;
            });
    ...
    ngObibaMica.search.component("vocabularyFilterDetail", {
        transclude: true,
        bindings: {
            vocabulary: "<",
...
(function() {
    ...
    function CriterionDropdown($document, $timeout, ngObibaMicaSearchTemplateUrl) {
        return {
            ...
            controller: [ "$scope", "$filter", "LocalizedValues", "VocabularyService", "StringUtils", CriterionDropdownController ],
            templateUrl: ngObibaMicaSearchTemplateUrl.getTemplateUrl("CriterionDropdownTemplate"),
            link: function($scope, $element) {
                var onDocumentClick = function(event) {
                    var isChild = document.querySelector("#" + $scope.criterion.id.replace(".", "-") + "-dropdown-" + $scope.timestamp).contains(event.target);
                    if (!isChild) {
                        $timeout(function() {
...
(function() {
    ...
    ngObibaMica.search.controller("CriterionLogicalController", [ "$scope", function($scope) {
...
ngObibaMica.search.controller("MatchCriterionTermsController", [ "$scope", "RqlQueryService", "LocalizedValues", "JoinQuerySearchResource", "RqlQueryUtils", "SearchContext", function($scope, RqlQueryService, LocalizedValues, JoinQuerySearchResource, RqlQueryUtils, SearchContext) {
...
ngObibaMica.search.controller("NumericCriterionController", [ "$scope", "RqlQueryService", "LocalizedValues", "JoinQuerySearchResource", "RqlQueryUtils", "SearchContext", function($scope, RqlQueryService, LocalizedValues, JoinQuerySearchResource, RqlQueryUtils, SearchContext) {
    $scope.lang = SearchContext.currentLocale();
    ...
    var updateLimits = function() {
        ...
        JoinQuerySearchResource[targetToType(target)]({
            query: joinQuery
        }).$promise.then(function(joinQueryResponse) {
...
ngObibaMica.search.controller("searchCriteriaRegionController", [ "$scope", "RqlQueryService", "$timeout", function($scope, RqlQueryService, $timeout) {
    var canShow = false;
...
ngObibaMica.search.controller("StringCriterionTermsController", [ "$scope", "RqlQueryService", "LocalizedValues", "StringUtils", "JoinQuerySearchResource", "RqlQueryUtils", "SearchContext", "$filter", function($scope, RqlQueryService, LocalizedValues, StringUtils, JoinQuerySearchResource, RqlQueryUtils, SearchContext, $filter) {
    $scope.lang = SearchContext.currentLocale();
